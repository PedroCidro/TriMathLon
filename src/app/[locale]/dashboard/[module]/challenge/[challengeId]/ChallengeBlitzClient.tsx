...
import { ArrowLeft, Swords, RotateCcw, Trophy, Loader2, Crown } from 'lucide-react';
...
export default function ChallengeBlitzClient({
    ...
    const [gameState, setGameState] = useState<ChallengeGameState>(
        challengeStatus === 'playing' ? 'loading' : 'loading'
    );
    ...
    useEffect(() => {
        async function init() {
            ...
            if (data?.status === 'playing' && data.game_started_at) {
                // Game already in progress, jump to playing
                gameStartedAtRef.current = data.game_started_at;
                const elapsed = Math.floor((Date.now() - new Date(data.game_started_at).getTime()) / 1000);
                setTimeLeft(Math.max(0, gameDuration - elapsed));
                setGameState('playing');
    ...
    }, [gameState, isCreator, challengeId]);
    ...
    // Poll opponent during playing/waiting states
    useEffect(() => {
    ...
    const handleOptionSelect = (index: number) => {
        ...
        // Send score update every 2 answers
        if (answerCountRef.current % 2 === 0) {
            // Use latest values
